...
it; the card polls that single property and calls `refreshObjectProperties()`
as soon as its value differs from the previous one. Displayed properties update
in place, with no page reload and no user action.
...
| `POLL_INTERVAL_MS` | `5000` | Poll period, in milliseconds |
